...
(flet ((symbf (package name)
  ...
  (defmethod asdf:perform ((op asdf:compile-op) (c (eql (asdf:find-system :qt-libs))))
    (funcall (symbf :qt-lib-generator :ensure-libcommonqt))
    (funcall (symbf :qt-libs :ensure-standalone-libs)))

  (defmethod asdf:perform ((op asdf:load-op) (c (eql (asdf:find-system :qt-libs))))
    (asdf:load-system :qt))

  (defmethod asdf:perform ((op asdf:load-op) (c (eql (asdf:find-system :qt))))
